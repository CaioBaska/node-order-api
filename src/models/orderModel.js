...
function transformOrderData(input) {
  return {
    ...
    creationDate: new Date(input.dataCriacao).toISOString().slice(0, -1) + 'Z',  // Normaliza data
...
function getOrderById(orderId, callback) {
  db.get('SELECT * FROM "Order" WHERE orderId = ?', [orderId], (err, order) => {
    if (err || !order) return callback(err || new Error('Order not found'));
    db.all('SELECT productId, quantity, price FROM Items WHERE orderId = ?', [orderId], (err, items) => {
      if (err) return callback(err);
...
function listOrders(callback) {
  db.all('SELECT * FROM "Order"', (err, orders) => {
    ...
    orders.forEach(order => {
      db.all('SELECT productId, quantity, price FROM Items WHERE orderId = ?', [order.orderId], (err, items) => {
...
function updateOrder(orderId, updateData, callback) {
  ...
  db.run(
    ...
    function (err) {
      if (err || this.changes === 0) return callback(err || new Error('Order not found'));
